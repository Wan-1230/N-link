...
class ConnectionHealthWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            when {
                // 完全断开 → 尝试重连
                state == com.nikonlink.app.device.model.ConnectionState.DISCONNECTED -> {
                    Timber.tag(TAG).w("Connection lost, attempting auto-reconnect")
                    connectionManager.reconnectLastDevice()
                    Result.success()
                }
                // BLE 连接但无 WiFi → 正常（低功耗模式）
                state == com.nikonlink.app.device.model.ConnectionState.BLE_CONNECTED -> {
                ...
                // 错误等待重试 → 触发重连
                state == com.nikonlink.app.device.model.ConnectionState.ERROR_WAITING_RETRY -> {
