...
- **在 `fontScale ≥ 1.3` 时，所有 L2 玻璃胶囊从"固定高 + 单行"改为"垂直内衬 + 允许换行"** —— 否则玻璃按钮文字必被裁。写进 AC-11。
  **部分落地（§15.5m）**：`GlassTokens.largeFont()` + `View.growForLargeFont()`，已接相册页分段胶囊
  （40dp→54dp；网格内衬按标签行实测高算，所以列表让位自动跟上）。**未接**：监看 HUD 四枚 38dp、
  拍摄页模式槽 34dp、设备页模式行 44dp、支持页三枚 36dp —— 前三处的高度被滑块宽度计算间接依赖，
  必须真机看一帧才敢定。低于 1.3 档完全走原路径，不动任何像素。
...
> ⚠ **2026-09-21 复核订正（PRD v2.5 §2.3）**：13 条 AC 里，本次只逐条核了能静态判定的部分，
> 其余**不下结论**——本文当年的「可签」口径不可信，具体：
> · **AC-1 未达成（已证伪）** —— 实测硬编码色值 **54 处**（`res/layout` + `values/colors.xml`，分布在 6 个文件）+ Kotlin **36 处**，而本条要求 **= 0**。
> · **AC-8 未验证（已证伪）** —— 玻璃相关单测 **0 个**（`app/src/test` 下无 glass 目录），「20 次连续拖选」既无自动化也没留下真机记录。
> · **AC-2 / AC-5 / AC-6 / AC-7 / AC-9 / AC-10 / AC-11 未执行** —— 全部依赖真机与截图基线，仓库里找不到任何对应产物（截图基线、batterystats、Monkey 记录均无）。
> · **AC-3 / AC-4 / AC-12 / AC-13 本次未逐条复核**，不给结论；其中 AC-12 的「逐像素等于 v2.2」在无真机截图的条件下**原理上无法自证**。
> 结论：**v2.3 的「视觉改版验收全部通过」这句话不成立**（至少 2 条已被数字证伪、7 条从未执行），v2.5 不再复述该口径。
...
### 15.5m 第十四包（部分）：大字档胶囊 + 收尾清单

AC-4 / AC-7 / AC-11 / AC-13 与 D2 / D6 推到这里之后，剩下的条目按**「能不能自证正确」**分两类，
这个分类决定了它们现在做还是等有设备再做：

- **可以闭着眼睛做的 —— 已全部做完**：`NlFeedback` 收口 42 处（D6）、真模糊面配额断言（AC-4）、
  传输期降级（AC-7）、系统高对比度自动触发（AC-11 前半）、顶栏触点补足 48dp（AC-11 中）、
  大屏 dock 收宽 480dp（AC-13 前半）、大字档胶囊长高的一处（§7.3 前半）。
  共同点：对错是**行为契约或尺寸常量**，读代码 + 编译 + 单测就能定，不需要看。
- **必须用眼睛判的 —— 刻意没盲改**：M5 弹簧与深度转场、D2 全局 edge-to-edge、≥840dp 竖排侧栏、
  连续曲率圆角、sheen/tilt、`EXIF` sheet 组件化、其余大字档胶囊。
  这四类各自要动转场栈 / 窗口 insets / 整页布局 / 材质路径，而改错的症状 —— 内容顶进状态栏、
  共享元素落点偏一格、侧栏压住列表首列、圆角处出现台阶 —— 全是只有真机才看得见的那类。
  本机既没有设备、模拟器又起不来（§15.5g 有完整记录），所以**不交"编译通过但视觉未知"的东西**，
  判断权留到走查。这一条是主动取舍，不是进度虚报。
