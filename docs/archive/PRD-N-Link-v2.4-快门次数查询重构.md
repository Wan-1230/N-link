...
2. **§L1 的 `OperationsSupported` 能力协商不做**，且 `db8a2a2` 起连「会话内降级标志」也不要了：
   `parseDeviceInfo` 是监看/相册/参数共享的路径，为省一次往返去改它不值。
   失败模式改由**阶段划分**承担 —— 局读逐样张试完（最多 3 张，每张 1MB），一次都没读到才整文件下载
   **一张**；因此不需要跨样本记住「这台机身不支持局读」。**限制的是下载次数而不是字节数**，
   原方案设想的 2MB 硬上限没有实现：截断的对象喂不进云端 EXIF 接口，也喂不进本机解析，是个假优化。
